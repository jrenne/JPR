---
title: "Affine modeling of credit risk, pricing of credit events, and contagion"
author: "Alain Monfort, Fulvio Pegoraro, Jean-Paul Renne, Guillaume Roussellet"
description: "Management Science"
date: "10/5/2020"
categories: [term structure, credit risk, published]
image: "figure.pdf"
---

<!-- description: "Affine Term Structure Model that provides non-negative yields and is able to accommodate a short-term rate that stays at the zero lower bound (ZLB) for extended periods of time." -->

<!-- subtitle: -->

<!-- image: "figure.jpeg" -->


We propose a discrete-time affine pricing model that simultaneously allows for (i) the presence of systemic entities by departing from the no-jump condition on the factors’ conditional distribution, (ii) contagion effects, and (iii) the pricing of credit events. Our affine framework delivers explicit pricing formulas for default-sensitive securities such as bonds and credit default swaps (CDSs). We estimate a euro-area multicountry version of the model and address economic questions related to the pricing of sovereign credit risk. We find that both frailty (common factors) and contagion phenomena are important to account for the joint dynamics of credit spreads. Our results also provide evidence of credit-event pricing, which is at the source of substantial credit risk premiums, even for short maturities. Finally, we extract measures of depreciation-at-default from CDSs denominated in different currencies.






```{=html}
<div  style="margin: 30px; text-align: center;">
<a class="btn btn-primary" href="https://doi.org/10.1287/mnsc.2020.3658" role="button" target="_blank" style="            display: inline-block;
            padding: 10px 20px;
            border: 2px solid #333;
            border-radius: 20px;
            background-color: #f1f1f1;
            text-decoration: none;
            color: #333;
            margin: 0 10px;
            font-size: 16px;">Link to paper</a>
<a class="btn btn-primary" href="https://www.dropbox.com/s/1m0g9p4bt1vkhm2/Rcode_CDS.zip?dl=0" role="button" target="_blank" style="            display: inline-block;
            padding: 10px 20px;
            border: 2px solid #333;
            border-radius: 20px;
            background-color: #f1f1f1;
            text-decoration: none;
            color: #333;
            margin: 0 10px;
            font-size: 16px;">Link to replication code</a>
</div>
```



<!-- In order to have bibtex in place of R code, add in front matter: -->
<!-- format: -->
<!--   html: -->
<!--     code-fold: true -->
<!--     code-summary: "Bibtex" -->

<!-- ```{r essai, eval=FALSE} -->
<!-- article = { -->
<!--   author = {JPR}, -->
<!--   year=2010 -->
<!-- } -->
<!-- ``` -->
